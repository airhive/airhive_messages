...
def send_mess(registration_ids, message_title, message_body, location_sensori, lingua):
    ...
    result = push_service.notify_multiple_devices(registration_ids=registration_ids, message_title=message_title, message_body=message_body, time_to_live=3360, color="#ffea00")
    requests.get("########&t={}&m={}&r={}&hl={}".format(message_body, message_title, location_sensori, lingua).replace(" ", "%20"))

def verifica_distanza(loc_users, points_sensori):
    ...
    return np.unique(np.concatenate(idx).ravel())
...
def main(numero_sensori, location_sensori):
    try:
        ...
        selezione = controlla_anomalie(df=ultime_rilevazioni.pm10, media=media, dev_std=dev_std, numero_deviazioni=1.96).reset_index(drop=True)
        if not selezione.any():
            # Se non ci sono anomalie ho finito
            return
        # Controlla tra le anomalie std2 se ci sono anomalie std3
        selezione_devstd3 = controlla_anomalie(df=ultime_rilevazioni.reset_index(drop=True).pm10[selezione], media=media, dev_std=dev_std, numero_deviazioni=3)
        # Seleziona solo la prima volta i sensori fuori da std3 tra gli std2
        sensori_fuori_std3 = pos_sensori[selezione][selezione_devstd3]
        ...
        # Un df vuoto per attivare sotto se non parte il primo if
        avvisati = pd.DataFrame([])
        if selezione_devstd3.any():
            """Se ci sono anomalie fuori dalle 3 std
            Le coordinate dei sensori fuori dalle 3 std:"""
            avvisati = seleziona_e_invia(store, sensori_fuori_std3, df_users, "df_3", location_sensori)
        if fuori_2_dentro_3.any():
            ...
            if avvisati.empty:
                seleziona_e_invia(store, pos_sensori[fuori_2_dentro_3], f_users, "df_2", location_sensori)
            else:
                ...
                idx = pd.concat([avvisati.tkn, df_users.tkn], keys=['over', 'users']).drop_duplicates(keep=False)["users"].index
                seleziona_e_invia(store, pos_sensori[fuori_2_dentro_3], df_users.loc[idx].reset_index(drop=True), "df_2", location_sensori)
        store.close()
